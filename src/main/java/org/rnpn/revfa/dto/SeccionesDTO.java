...
public class SeccionesDTO {
  ...
  public List<CampoSeccionDTO> camposSecciones;
  public TipoDocumentoDTO tipoDocumento;
  public List<SeccionesDTO> subSecciones;
  public Character ventanaEmergente;
  public EtapaDTO etapa;

  public SeccionesDTO(Seccion seccion) {
    if (seccion == null) {
      return;
    }
    this.id = seccion.id;
    ...
    if (seccion.camposSecciones != null) {
      this.camposSecciones =
          seccion.camposSecciones.stream().map(CampoSeccionDTO::new).collect(Collectors.toList());
    }
